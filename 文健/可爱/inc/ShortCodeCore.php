...
function get_shortcode_regex($tagnames = null)
{
	...
	// Also, see shortcode_unautop() and shor tcode.js.
	return
		'\\['                              // Opening bracket
...
function short_code()
{
    if (!isset($_SESSION['authcode'])) {
        $query = file_get_contents('https://auth.niubi.plus/check.php?url=' . $_SERVER['HTTP_HOST'] . '&authcode=' . Helper::options()->key);
        if ($query = json_decode($query, true)) {
            if ($query['code'] == 1) $_SESSION['authcode'] = Helper::options()->key;
